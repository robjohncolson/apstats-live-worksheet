...
**Last updated**: 2026-05-11 (post session 95 — registry plumbing cleanup: dead sidecars deleted, Supabase + roadmap re-bake wired into `lesson-prep.mjs`)
...
**Current focus**: backfilling U1/U2/U3 follow-along worksheets (registry shows 8+7+2 lessons stubbed from Schoology with `ingest=pending`, no worksheet HTML on disk yet) — see Open Carry-overs (h).
...
- **(h) [TOP PRIORITY] Backfill U1/U2/U3 follow-along worksheets** — registry has 17 lessons stubbed but never ingested: U1 (1.3-1.10, 8 lessons), U2 (2.1-2.7, 7 lessons), U3 (3.4, 3.5; 3.6-7 already done). Each needs the full pipeline (`node Agent/scripts/lesson-prep.mjs --unit U --lesson L`) which now (post-s95) auto-pushes to Supabase + re-bakes the roadmap. Schoology folders already scraped (`schoology=scraped` in registry), so video transcripts are the missing input. Pace: ~4 lessons/week realistic given each takes ~30min interactive. **Backfill kickoff command** (refreshes all three sinks from current registry state, no pipeline needed): `node Agent/scripts/sync-schedule-to-supabase.mjs --execute && node Agent/scripts/build-roadmap-data.mjs`.

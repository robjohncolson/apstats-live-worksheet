...
**➡ NEXT THREAD — Desk "Do Now" Sprint DN2 (TEACHER-GATED — do NOT start without explicit greenlight). Handoff written at context limit 2026-05-18.**
...
**NEXT = DN2 feeder/roster adoption (staged; teacher-gated; vocab now de-risked):** DN2a wire 1–2 worksheets, prove worksheet→`gradebookClient.record`→`item_ledger`→`/donow` loop → DN2b roll to all 69 (scripted, tight commit) → DN2c `rosterClient` single sign-in into the Desk `ap_stats_roadmap_square_mode.html` → DN2d curriculum_render quiz feeder (separate repo; answer-submit path only; NEVER touch sacred `curriculum.js`). THEN DN3 (collapse roadmap to ONE fall calendar Sept-1→exam + 4 completion-color states incl. "done-ahead" glow + Do Now card + soft speed-bump). THEN controlled full T2 disambiguation run (Codex pipeline; planner-review agreement+stratified sample BEFORE merging into `data/skill-map.json`; disagreements→Sprint T3) → Phase 2/3/4.
...
**Method:** freeze contracts → parallel Sonnet → Codex FOCUSED review (`python C:/Users/rober/Downloads/Projects/Agent/runner/cross-agent.py`; broad times out — keep tight + ~1400s) → planner re-verify under vitest (s88b: NEVER trust result files; agents stream-timeout on long runs but usually finish the work — verify on disk) → tight single-purpose commits (forensic HEAD before/after; clobber-free vs the other session). Baseline: root suite **1344/1345** (the 1 = pre-existing unrelated `study-guide.test.js` v3-structure — NOT a regression; do not "fix"); roster-server **80/80**.

**Open chores / flags:** (a) `delete from roster where section='SMOKETEST';` clears Phase-0/Sprint-1 test rows (FK cascade clears ledger too). (b) curriculum.js bank defects (~10 dup-MCQ-choice items + 2 prompt/rubric mismatches, list in `project_gradebook_grading_model.md`) — sacred, never auto-fix; teacher may clean separately. (c) apstat_5 cross-finding (AI-tutor: it wasn't truly `## TOPIC`-malformed; T1's TT1-E restructure may be moot) — verify T1 restructure faithfulness at the T2 review gate; does NOT affect shipped DN1. Recall: `project_desk_donow.md`, `project_gradebook_grading_model.md`, `project_gradebook_phase0.md`, `project_ai_tutor_pilot.md`.

# CONTINUATION PROMPT — REPO HYGIENE: edgar/MIT worksheet removal + Desk roadmap network resilience (s25) ; TETRIS HARDENING: Study Break 1v1 multiplayer + stakes robustness pass (s24) ; MODAL ESCAPE: every Desk content modal now closes with Esc (s21) ; AVATAR MENU: click classmate → name → 🍬 candy / ⚔️ game, click self → 🎉 happy bounce (s20/s22) ; STUDY BREAK STAKES LIVE: bet candy on best-of-3 Tetris (s19, backend+client) ; DOGE PRESENCE chips+submenu (s18) ; candy↔DOGE CONSERVATION AUDIT + F1 race FIXED (s17) ; DOGE ⇄ candy BIDIRECTIONAL (s16) ; candy economy REVIVED (s15) ; grade-integrity + calendar COMPLETE

> **AUTHORITATIVE. Supersedes everything below.** Last updated 2026-06-20 (session 25 — repo hygiene: removed edgar/MIT worksheets + added Desk roadmap network resilience; presence-disagreement bug + grade/flashcard concerns under investigation).
> follow-alongs HEAD = the s25 resilience commit `0555871` (edgar/MIT removal `9a9ae93`; this docs commit follows). Both s25 commits are FRONTEND-ONLY (no `roster-server/**` or cr change → no backend deploy). Earlier session anchor was the s24 MP-5 heartbeat `1ad4d4e`. Repo `apstats-live-worksheet`, branch `master`. **GH Pages auto-publishes `master`**.
...
## ⏭ SESSION 25 (2026-06-20) — repo hygiene SHIPPED (edgar/MIT removal + Desk roadmap resilience); presence + grade concerns UNDER INVESTIGATION
...
**⏭ UNDER INVESTIGATION (teacher's actual concern — worried a Codex refactor broke things):**
1. **PRESENCE DISAGREEMENT (Desk):** a slightly-greyed avatar is clickable (shows name + actions) but that kid is NOT in the top-right 🐶 doge "Online Now" list. This is the REVERSE of the s18-documented asymmetry (s18 = doge-list-present / avatar-absent for off-Desk kids). Working hypothesis: the avatar feed (`classroom_join`, room-scoped) renders them while the global doge feed (`identify`→`user_online`, wired via the Desk's `_autoPresence`) does not — a boot-order / identify / stale-socket regression. [root-cause investigation in progress]
2. **GRADES — 100% despite no Blooket:** v3 `max(PC, Work)` masks the Blooket Work track BY DESIGN (see `project_blooket_necessity_decision.md`, 2026-06-19); not-yet-due lessons are out of the denominator → un-attempted DUE lessons become 0 as dates pass (to confirm to the teacher). Teacher now feels the flashcard→next-lesson GATE is "not a strong enough signal" — may be REOPENING the blooket-necessity decision. [discussion pending — do not silently re-propose a grade penalty; the teacher is driving this]

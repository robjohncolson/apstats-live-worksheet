...
**Last updated**: 2026-05-17 (session 99 **COMPLETE** — Gradebook **Phase 0 is FULLY LIVE end-to-end**: built (`8510252`), §6.1 revised to reuse curriculum_render Supabase (`eefe0ae`), **deployed to Railway & production-verified** (`a7d7bbd`, URL `https://roster-production-12c1.up.railway.app`). **Next thread: Phase 1** (item_ledger + worksheet/FRQ feeders carrying `student_id`) + single-sign-in adoption on the roadmap. See "SESSION 99 — COMPLETE" + `project_gradebook_phase0.md`.)
...
**➡ NEXT THREAD — Phase 1 (Phase 0 is DONE & live; do NOT rebuild or redeploy it).** Phase 0 shipped, §6.1-revised, deployed to Railway and production-verified (`a7d7bbd`; `https://roster-production-12c1.up.railway.app`). Phase 1 = create `item_ledger` (one row per student×gradeable item) in the curriculum_render Supabase project + wire the two easy feeders: worksheet fill-ins (Railway `/api/submit-answer`) and FRQ AI grades (`/api/ai/grade`/`/api/ai/appeal`) now carrying `rosterClient.studentId()` — the `student_id` field/contract is already fixed. Pair with the §6.4 adoption work: a single sign-in surface on the roadmap loading `roster_config.js`+`roster-client.js`, so worksheets/study guide inherit identity from the one `apstats_roster.v1` key. Respect sacred `curriculum.js` (Phase 2 quiz feeder is a new write path only). **Design input now exists:** `GRADEBOOK_GRADING_SPEC.md` (s99 DRAFT, **awaiting teacher sign-off**) specs the fall grade model (mastery/repeatable, gate+evidence, proctored-anchored BKT, computed-pre, learning-loop retakes, remediation record) — it's the Phase 3/4 design, **promotes Phase 2 onto the critical path**, and names the item→AP-skill tagging audit as the spine risk. Sign off + decide its §9 knobs before build-planning Phase 2+. User prefers brainstorm→spec→implement (brainstorm+spec DONE for grading). One chore: `delete from roster where section='SMOKETEST';` clears the Phase 0 verification rows. Recall: `project_gradebook_grading_model.md`. *(Historical Phase 0 handoff detail preserved below for reference; it is COMPLETE.)*
...
- **(j) ➡ ACTIVE — Bulletproof gradebook. Phase 0 FULLY LIVE (`a7d7bbd`, s99 — built+§6.1-revised+deployed+prod-verified); Phase 1 next.** Live auth service: `https://roster-production-12c1.up.railway.app` (Railway project `apstats-roster`/service `roster`) on the curriculum_render Supabase project. `ROSTER_TEACHER_SECRET=tagQc8e7mEXDUkqwYSYLqzH8` (enroll students). Chore: `delete from roster where section='SMOKETEST';`. Spec `GRADEBOOK_SPEC.md` signed off s98 (`67b28e9`); Phase 0 built/reviewed/deployed/prod-verified s99. **Do NOT rebuild or redeploy Phase 0.** **Phase 1** = `item_ledger` (curriculum_render Supabase) + worksheet/FRQ feeders stamped with `rosterClient.studentId()` (the `student_id` field contract is already fixed) + §6.4 single-sign-in adoption on the roadmap. Then Phase 2 (quiz feeder — new write path only, never touches sacred `curriculum.js`) → 3 (skill_mastery+grade) → 4 (teacher dashboard). Recall: `project_gradebook_phase0.md`.

...
> **AUTHORITATIVE. Supersedes everything below.** Last updated 2026-07-01 (session 32 — grade-integrity §0 fix SHIPPED; mobile launcher → Desk parity (sign-in + grade + Blooket + flashcards); QR engine built+shelved; RFCOMM rejected; 3 specs hardened). Prior s31 = ANDROID Phases 2-5 shipped + on-device validated (Nearby gossip Pixel3↔S24; Ph5 R2 video). s30 = Nightly Review all 4 phases.
> follow-alongs HEAD (pushed) = `7626846` (s32 tip). s32 fa commits: `75c9fea` gossip Nearby-perm fix (s31 tail, now committed), `6acf267` launcher diamond-dial sign-in + shows name, `7c8baaf` QR-sync spec, **`5cea79b` §0 signature↔grade BINDING fix (SHIPPED, grade-affecting — live on GH Pages + all 3 devices)**, `257cb3f` QR engine (fountain+DUMP, 25 tests, INERT), `40eefed` RFCOMM rejected + Ph6 status, `fb6ab10` launcher grade pill + ✓ badges + Blooket, `3ba6c6a` in-app flashcards (deep-link to Desk), `72bbd21` offline-grading-mesh spec, `7626846` nightly-review-v2 spec. cr HEAD = `1f446c7` (UNCHANGED this session — all s32 work is follow-alongs client/frontend, no roster-server change). Repo `apstats-live-worksheet`, branch `master`. **GH Pages auto-publishes `master`**; cr `railway-server/**` auto-deploys (untouched).
...
> **SPEC'D-BUT-NOT-BUILT (all reviewed + §0-hardened this session — build deliberately IF needed):** (1) **QR ledger-sync** `ANDROID_PHASE6_QR_SYNC_SPEC.md` — §0 SHIPPED; the fountain+DUMP ENGINE is built+tested but INERT (no render/camera/UI); RFCOMM alternative REJECTED (`ANDROID_PHASE6B_QR_BT_SPEC.md §0`: not faster than Nearby + a per-session discoverable prompt + broadcast-QR-key). (2) **Nightly Review v2** `NIGHTLY_REVIEW_V2_SPEC.md` — AI-drafted comments (gate to AI-graded sources only!) + review-by-item lens. (3) **Offline-grading mesh** `OFFLINE_GRADING_MESH_SPEC.md` — the raw-work lane / student PKI (⚠ naive design NOT shippable; §0 has the 11 fixes: domain-separate `verifyLedgerRow` by `t`, authenticate pubkey→sid, student-key revocation, grade-gated suppression not pruning, deterministic receipts). Transport stack DECISION: STOP — use **Nearby** (`GossipNearby`, tested) + animated-QR floor engine + roster server (online hub).

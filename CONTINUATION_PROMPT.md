...
> follow-alongs HEAD = `3a0be9b` (s31 tip; s31 fa: `4bb5810` Phase 2 on-device ledger merge, `2873da3` Phase 3 P2P gossip, `13e6f0d` continuation→s31, `3a0be9b` Phase 4 FOUNDATION = issuer-key history/rotation). cr HEAD = `1f446c7` (UNCHANGED this session — Phase 2/3/4 are follow-alongs-only). s30 tip was fa `7119dcc`. Repo `apstats-live-worksheet`, branch `master`. **GH Pages auto-publishes `master`**; cr `railway-server/**` auto-deploys to the cr Railway server, cr root → cr GH Pages (the quiz app).
...
- **PHASE 4 — FOUNDATION SHIPPED (`3a0be9b`)**: the **issuer-key history/rotation** is built + tested
  ...
  - ⏭ **DECISION PENDING (spec §3, gates the native work): where does the teacher key live + sign?**
    (A) server stays sole signer (simplest, no offline epoch); (B) ⭐**device-GENERATED hardware
    Keystore key, pubkey added to the trust set** (recommended — TRUE hardware signing, reuses the
    history, SIDESTEPS the largely-infeasible import of the existing Ed25519 key into Keystore; Railway
    co-signs + durable fallback; key loss = non-event); (C) software key (copy of existing key)
    encrypted at rest, signed in app code. **⚠ Original spec §8 "(c) copy `RECEIPT_ISSUER_PRIVATE_KEY`
    into Keystore" is largely INFEASIBLE** — Android Keystore doesn't import external Ed25519 keys on
    most devices. After the decision: native keygen + `BiometricPrompt` + a `sign(payload)` Capacitor
    method + the import→grade-pending-FRQ→sign→seal-epoch→push teacher-loop UI.

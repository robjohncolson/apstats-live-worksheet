...
1. **RUN migration `roster-server/migrations/0036_item_ledger_bonus_source.sql`** in Supabase (adds `bonus` + `bonus_applied`
   to the `item_ledger` source CHECK). Until then `POST /class/bonus` / `apply-bonus` answer 503 "run migration 0036";

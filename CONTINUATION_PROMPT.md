...
> follow-alongs HEAD (pushed) = `95e41cf` (s31 tip; **ANDROID PHASES 2-5 ALL SHIPPED; Ph5 video + Ph3 gossip VALIDATED ON-DEVICE**; s31 fa: `4bb5810` Ph2 ledger merge, `2873da3` Ph3 P2P gossip, `3a0be9b`+`e0fbbf7`+`0f4f16e` Ph4 teacher app, `1dd2204`+`86f1f26` Ph5 Play video+R2 tooling, `8ec20fd` continuation, `95e41cf` Ph3 Sync-Nearby on the phone launcher). cr HEAD = `1f446c7` (UNCHANGED this session). Repo `apstats-live-worksheet`, branch `master`. **GH Pages auto-publishes `master`**; cr `railway-server/**` auto-deploys.
> ⚠⚠ **UNCOMMITTED (s31 tail — MUST COMMIT):** the gossip NEARBY-PERMISSION FIX + logging is NOT yet committed. Files: `android-app/plugins/gossip-nearby/android/src/main/java/.../GossipNearbyPlugin.java` (added `Log.d` to all callbacks incl. onConnectionResult status; the alias KEPT all 6 perms), `android-app/plugins/gossip-nearby/android/src/main/AndroidManifest.xml` (**removed `maxSdkVersion="32"` from ACCESS_FINE/COARSE_LOCATION** — the real fix), `mobile-home.html` (gossip `durationMs` 20000→45000). **THE BUG (found on-device, tests couldn't):** Nearby Connections `startDiscovery` REQUIRES `ACCESS_COARSE_LOCATION` at runtime even on Android 14 (fails `8034 MISSING_PERMISSION_ACCESS_COARSE_LOCATION`; `neverForLocation` does NOT exempt it) — but the manifest capped location at API 32, so on Android 13+ it wasn't held → `start()` failed silently → "no nearby devices". Fix = declare location WITHOUT maxSdk + keep it in the @CapacitorPlugin "nearby" alias. **After fix: 2-DEVICE GOSSIP WORKS** — Pixel 3 (API 31, serial `89VX0HPX1`) + Galaxy S24 (API 34, `RFCX81GNBRE`) discovered + connected over BT + ran HELLO→SYNC→SEND + transferred ~40KB of signed rows (logcat: `onPayloadReceived KZDM (40086 chars)`, banner "📡 Connected — syncing…"). Was verifying the final "+N records" count when context filled. Also the Pixel tablet = `47251HFH807FLS`. **On-device driving:** pre-grant perms via `adb -s <ser> shell pm grant com.robcolson.apstats android.permission.{BLUETOOTH_SCAN,BLUETOOTH_ADVERTISE,BLUETOOTH_CONNECT,NEARBY_WIFI_DEVICES,ACCESS_FINE_LOCATION,ACCESS_COARSE_LOCATION}` + `svc bluetooth enable`; the fixed Play APK is at `android-app/android/app/build/outputs/apk/debug/app-debug.apk` (rebuild: `node scripts/build-android.mjs --play --media-base "https://pub-c41e030062804d678d5f79d5440fdd45.r2.dev"` then `cd android-app/android && rm -f app/build/outputs/apk/debug/app-debug.apk && ./gradlew.bat assembleDebug --no-daemon` for an installable APK — the AAB itself can't be adb-installed).

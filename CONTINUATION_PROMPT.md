...
**Last updated**: 2026-05-16 (post session 96 — Unit 1 follow-along backfill COMPLETE: all 10 lessons live + registered, then fully re-generated with AP framework guidance; AI Studio attach bug fixed; framework-injection bug fixed)
...
**Current focus**: follow-along backfill — **Unit 1 (1.1–1.10) DONE**: all live at `robjohncolson.github.io/apstats-live-worksheet/u1_lesson{N}_live.html`, registered, Supabase-upserted, and framework-anchored. Next: **Unit 2 (2.1–2.7+)**, then U3 (3.4, 3.5). Recipe is fully mechanical — see Open Carry-overs (h).
...
- **(h) [TOP PRIORITY] Backfill follow-along worksheets — U1 DONE, U2/U3 next.** Unit 1 (1.1–1.10) is fully complete: live, registered, Supabase-upserted, framework-anchored. Remaining: **Unit 2 (2.1–2.7+)** then **U3 (3.4, 3.5)** (3.6–3.7 already done). The recipe is now fully mechanical (~15 min Codex build/lesson):
  ...
  - **Gotchas:** `codex-content-gen.mjs` has no resume guard but the *task-runner* skips when registry status=done — to re-generate a finalized lesson use `--force-step content-gen-worksheet --force-step content-gen-blooket` (task-IDs, not the CLI-documented `worksheet`/`blooketCsv`). A 3-video lesson exceeds the 20-min task-runner timeout → pipeline reports FAILED but Codex completed clean artifacts (HTML closed) → finalize manually. `apstat_2_framework.md` is bold-format so framework works there regardless; the f0d9809 fix is committed anyway. Full per-lesson detail + Drive IDs in auto-memory `project_aistudio_ingest_drive_picker.md`.

...
> follow-alongs HEAD = `0f4f16e` (s31 tip; s31 fa: `4bb5810` Phase 2 ledger merge, `2873da3` Phase 3 P2P gossip, `3a0be9b`+`e0fbbf7` Phase 4 issuer-key-history+signing-core, `0f4f16e` Phase 4 FULL teacher app). cr HEAD = `1f446c7` (UNCHANGED this session — Phase 2/3/4 are follow-alongs-only). s30 tip was fa `7119dcc`. Repo `apstats-live-worksheet`, branch `master`. **GH Pages auto-publishes `master`**; cr `railway-server/**` auto-deploys to the cr Railway server, cr root → cr GH Pages (the quiz app).
...
- **PHASE 4 — FOUNDATION SHIPPED (`3a0be9b`)**: the **issuer-key history/rotation** is built + tested
  ...
  - **PHASE 4 FULLY SHIPPED (`0f4f16e`)** — the on-device TEACHER APP, built understand→design→
    ...
    - ⚠ **migration 0026 USER-RUN pending** (until run, register/revoke → 503). ⚠ **on-device runtime
      test pending** (biometric keygen/unlock + the grade→push→seal loop — only the user can do this).
      Tests: root 7642 + (ledger-seal 9, secure-key 7, teacher-app 7), roster-server 1153/1153.

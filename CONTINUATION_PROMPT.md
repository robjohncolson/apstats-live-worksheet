...
> follow-alongs HEAD = the s25 doge-heartbeat fix `5c230c3` (edgar/MIT removal `9a9ae93`, roadmap resilience `0555871`, docs `88220f2`). cr HEAD = `3ba6401` (classroom OFFLINE_GC fix → Railway auto-deploys). Earlier session anchor was the s24 MP-5 heartbeat `1ad4d4e`. Repo `apstats-live-worksheet`, branch `master`. **GH Pages auto-publishes `master`**; the cr classroom fix auto-deploys to the cr Railway server.

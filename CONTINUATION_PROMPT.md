...
**Last updated**: 2026-05-17 (session 99 **COMPLETE** — Gradebook **Phase 0 BUILD shipped & pushed** (`8510252`): roster-server auth service + `0001_roster.sql` + `roster-client.js`/`roster_config.js` + Decision Log. **Next thread: the Phase 0 live-provisioning USER-ACTION HANDOFF, then Phase 1.** See "SESSION 99 — COMPLETE" block below + `roster-server/README.md`.)
...
**Current focus**: **Gradebook Phase 0 is code-complete & pushed** (`8510252`). Remaining for Phase 0 = the **live-provisioning user-action handoff only** (new Supabase project + Railway deploy + `roster_config.js` URL — runbook in `roster-server/README.md`, decision D-F). Then **Phase 1** (item_ledger + worksheet/FRQ feeders stamped with `student_id`). Carry-over (h) DONE; roadmap U1–U5 summer-prep (`27bc1df`). U4/U5 per-lesson backfill **de-scoped**.
...
**➡ NEXT THREAD — Phase 0 live-provisioning HANDOFF (user action), then Phase 1.** Phase 0 code is DONE & pushed (`8510252`) — do **not** rebuild it. The only thing left for Phase 0 needs the user's accounts (decision D-F, full runbook in `roster-server/README.md`): (1) create the new dedicated Supabase project; (2) run `roster-server/migrations/0001_roster.sql` in its SQL editor; (3) create a Railway service from `roster-server/`, set env `ROSTER_SUPABASE_URL` / `ROSTER_SUPABASE_SERVICE_KEY` / `ROSTER_TOKEN_SECRET` / `ROSTER_TEACHER_SECRET`; (4) put the deployed URL in `roster_config.js`; (5) smoke-test (`curl /health`, enroll one student with the teacher secret, `signIn` from `roster-client-demo.html`). **Then Phase 1** = `item_ledger` table + the two easy feeders (worksheet fill-ins via Railway `/api/submit-answer`, FRQ AI grades via `/api/ai/grade`) now stamped with `rosterClient.studentId()`; the `student_id` field contract is already fixed. Then Phase 2 (curriculum_render quiz feeder — new write path only, never touches sacred `curriculum.js`) → Phase 3 (skill_mastery rollup + grade calc) → Phase 4 (teacher dashboard). User prefers brainstorm→spec→implement.
...
- **(j) ➡ ACTIVE — Bulletproof gradebook. Phase 0 BUILD DONE (`8510252`, s99); Phase 1 next.** Spec `GRADEBOOK_SPEC.md` signed off s98 (`67b28e9`); Phase 0 code shipped s99 (roster-server auth service + `0001_roster.sql` + `roster-client.js`/`roster_config.js`, all tests green, Codex-reviewed). **Do not rebuild Phase 0.** Remaining for Phase 0 = the user-action handoff only (provision Supabase project + Railway deploy + `roster_config.js` URL — runbook `roster-server/README.md`, decision D-F). **Phase 1** = `item_ledger` + worksheet/FRQ feeders stamped with `rosterClient.studentId()` (the `student_id` field contract is already fixed). Then Phase 2 (quiz feeder — new write path only, never touches sacred `curriculum.js`) → 3 (skill_mastery+grade) → 4 (teacher dashboard). Recall: `project_gradebook_phase0.md`.

...
> **AUTHORITATIVE. Supersedes everything below.** Last updated 2026-07-01 (session 33 — NIGHTLY REVIEW v2 SHIPPED (AI-drafted comments + by-item lens + 409-stale pins); OFFLINE-GRADING MESH Phases 1-2 SHIPPED (student PKI + domain separation + the submissions lane); GitNexus index refreshed). Prior s32 = grade-integrity §0 binding + mobile launcher → Desk parity + QR engine shelved. s31 = ANDROID Phases 2-5. s30 = Nightly Review v1.
> follow-alongs HEAD (pushed) = `acf03aa` (s33 tip). s33 fa commits: **`dbd7c0c` Nightly Review v2** (roster `/class/review-item/:ledgerId` AI-graded-only full response + `/class/review-by-item` + 409-stale rh/score pins + `rh` in the signed receipt + 14-day window; Desk By-item lens + ✨ Draft + cluster draft; grade-adjacent, roster-server AUTO-DEPLOYED), **`3a446dc` mesh Phase 1** (client `verifyLedgerRow`/`registerStudentKeys`/`verifySubmissionRow` in receipt-verify.js + `student-key.js`; server `snapshot-verify.js` t-check + `student-keys.js` + migration 0027 + db DAOs; grade-affecting, AUTO-DEPLOYED), **`acf03aa` mesh Phase 2** (client-only: `submission-store.js` + `submission-capture.js` + `ledger-gossip.js` lane tags/runLanes + mobile-home two-lane sync). cr HEAD = `6313293` (s33: **`6313293` `POST /api/ai/review-comment`** — the NRv2 comment drafter, DeepSeek-primary/Groq-failover, ≤180 char, no identifiers in the envelope; the user's uncommitted `max_tokens` WIP was kept OUT + re-applied). Repo `apstats-live-worksheet`, branch `master`. **GH Pages auto-publishes `master`**; **roster-server/ AND cr `railway-server/**` BOTH auto-deploy on push** (both touched this session — grade/AI-affecting).
> **⚠ s33 USER-ACTION ITEMS:** (1) `TEACHER_KEY` Railway env is set to the PUBLISHED default `apteacher2627` = zero secrecy — change it to a non-repo value if the NRv2 full-response endpoint should be genuinely gated (low-stakes threat model → optional). (2) migration `0027_student_keys` RAN ✅ (`/student-keys` live). (3) ⚠⚠ **the offline-grading mesh (Phases 1-2) is RUNTIME-UNTESTED — a 2-PHONE + teacher-device adb test is required before Phases 3-4 are built on top.** (4) known-flaky root test: `tests/teacher-student-console-dashboard-deeplink.test.js` (a jsdom load-event race, passes ~2/3 in isolation, untouched by s33) — the quoted s33 root count was 7748/7749 with that as the sole failure.
...
> **SPEC-STATUS (updated s33):** (1) **QR ledger-sync** `ANDROID_PHASE6_QR_SYNC_SPEC.md` — STILL SHELVED: §0 SHIPPED; fountain+DUMP ENGINE built+tested but INERT (no render/camera/UI); RFCOMM alternative REJECTED (`ANDROID_PHASE6B_QR_BT_SPEC.md §0`). Don't re-propose RFCOMM. (2) **Nightly Review v2** `NIGHTLY_REVIEW_V2_SPEC.md` — ✅ **SHIPPED s33** (see title; spec header marked implemented). (3) **Offline-grading mesh** `OFFLINE_GRADING_MESH_SPEC.md` — ⏳ **Phases 1-2 SHIPPED s33, Phases 3-4 NOT built** (teacher offline grade loop w/ deterministic receipts + anti-farming; hub reconcile). ⚠ RUNTIME-UNTESTED — 2-phone/teacher-device test needed before 3-4. Transport DECISION unchanged: **Nearby** (`GossipNearby`, tested) + animated-QR floor engine + roster server (online hub).

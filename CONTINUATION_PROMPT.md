...
> follow-alongs HEAD = `e0fbbf7` (s31 tip; s31 fa: `4bb5810` Phase 2 on-device ledger merge, `2873da3` Phase 3 P2P gossip, `3a0be9b` Phase 4 issuer-key history, `e0fbbf7` Phase 4 on-device Ed25519 SIGNING core). cr HEAD = `1f446c7` (UNCHANGED this session — Phase 2/3/4 are follow-alongs-only). s30 tip was fa `7119dcc`. Repo `apstats-live-worksheet`, branch `master`. **GH Pages auto-publishes `master`**; cr `railway-server/**` auto-deploys to the cr Railway server, cr root → cr GH Pages (the quiz app).
...
- **PHASE 4 — FOUNDATION SHIPPED (`3a0be9b`)**: the **issuer-key history/rotation** is built + tested
  ...
  - ⏭ **REMAINING Phase 4 (large, DEVICE-TESTED, can't runtime-validate on this box)** — Task: native
    `secure-store` Capacitor plugin (EncryptedSharedPreferences w/ a hardware Keystore MasterKey +
    `BiometricPrompt`) to hold the wrapped Ed25519 private JWK; a teacher "Signing Key" surface
    (generate via ReceiptSign → store via secure-store → register the device pubkey into the trust set,
    i.e. add to Railway `RETIRED_ISSUER_PUBKEYS` or a persisted trusted-issuers list); then the teacher
    grading-loop UI (import bundle → grade pending FRQs → sign → seal daily epoch → push/gossip out).

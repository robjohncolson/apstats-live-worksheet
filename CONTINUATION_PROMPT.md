...
## ⏭ SESSION 41 (2026-09-22 → 09-23, Athena laptop) — **CURRENT. Supersedes s40 below. BONUS BANK BUILT + weekly objectives + Blooket zeros + bulletin app.**

**HEAD = `35c8510e`** (origin/master, pushed). Roster-server auto-deployed. Two Codex (gpt-6-astra) implement rounds + one
adversarial review + one fix round via `Agent/runner/cross-agent.py`; Fable orchestrated, did Desk/UI edits, and folded.
Memory has the durable notes (`project_bonus_bank_policy`, `feedback_grading_philosophy_tilt_to_a`, `project_teacher_week_objectives`,
`project_blooket_masking_sim`, `project_flashcard_finish_outcome`, `project_section_break_days`, `project_desk_school_bulletin`).

### ⚠ s41 USER-ACTION CHECKLIST (live to-do)
1. ✅ **Migration 0036 RAN 2026-09-23** (teacher). Verified on the deployed server: `POST /class/bonus` (empty batch) and
   `POST /class/quarter/apply-bonus` (dry run) both answer 200. The source CHECK is first exercised by the first real
   `--apply`; the entry script dry-runs before writing.
2. **Hand-grade "Screen Time, Two Deletions"** (scan `20260921_144743.PDF`; Fable's second read `screen_time_grading.pdf/.tex`,
   11 students, 2 borderlines: Damian E-or-P, Yolandys P+-or-E). Resolve the no-period sheet (Elmer) and the unreadable
   "Mr. Co…" (B). Then enter with `node scripts/enter-bonus.mjs <file-outside-repo>` (dry run) → `--apply`. File format:
   `sheet=U1-screen-time quarter=Q1 title=Screen Time, Two Deletions` then `Real Name|E` lines. Refuses ambiguous names.
3. Watch the **first Blooket zeros** land: B 1.1 at the 09-23 16:30 sync (2 students), E 1.1 at 16:45 (incl. Jesselly).
   The 09-22 16:30 run predates the commit (no zeros) — that was not a bug.
4. **Decide the max(PC, Work) masking fix** (sim table in memory `project_blooket_masking_sim`): recommended `finalSlice`
   (Blooket 10% of the FINAL grade outside the max → flat ~8-pt cost of skipping all decks at any PC). Teacher said
   "tackle later"; needs a spec + grade-sim invariants rerun; grade-affecting.
5. Carry-forward from s40 (unchanged): Schoology rig items, `PAYOUT_AGENT_KEY` paste on Athena, `git branch -D codex-athena-rounds`.

### Shipped this session (all pushed, all tests green except the pre-existing `misconceptions-triage` date test + nothing else)
- **Bonus Bank** (`04c3d6f2` backend, `35c8510e` frontend; spec `BONUS_BANK_SPEC.md`, review `state/bonus-bank-review-1.json`).
  Teacher policy: hand-graded E/P/I → +5/+3/+1, BANKED as ledger rows the engine ignores everywhere (grade, mastery, rollup,
  Nightly Review, effort points), applied at quarter close to the **Work track only — never PC**, allowed to flip the 40%
  floor; placement rule NOT published (students' `bonus_applied` reads are redacted to grade+date). Routes: `POST /class/bonus`,
  `GET /class/quarter/deltas` (+`bonus`, +`closed` = the number to enter in Schoology), `POST /class/quarter/apply-bonus`
  (dryRun default, atomic first-writer, preserves the frozen early-bonus residual, ⚠ stale if re-frozen). Dashboard Quarter
  Close card: Banked / Applied / Closed columns + "Apply banked bonus…" preview → Confirm. Desk My Ledger: "Bonus banked"
  block, footer exactly "Applied at the end of the quarter." Grading philosophy recorded: tilt everything toward an earned A;
  leniency lives in recovery paths + teacher discretion, never as a published rule; PC stays the honest mastery number. Do
  NOT propose a mastery gate on the A before Q1 PC data (November) and only if the teacher raises it.
- **Blooket zero** (`805ed20e`): the Desk lesson gate is gone (since `1e2d6178` 09-10), so a missing Blooket is now a lagged
  0 in the Schoology column too (`tools/schoology_components.py`, same 13-day zero date; Quiz never zeroed) and the Desk
  zero-warning badge/card list unplayed decks ("Flashcards Topic X" → opens the deck). Engine already zeroed them.
- **Flashcard finish screen** (`d4554599`): says what happened to the grade (saved / best stays / not recorded) with ONE
  "Done" button; the commit patches `_gradeLessonsCache` and repaints the ledger card+badge at once (teacher confusion report).
- **Weekly objectives** (`e9ac8229`, `c88e4b43`, `140e52f8`): `teacher-week.html` = Mon–Fri per period with each video's
  "what will you learn" bullets (`data/video-objectives.json` ← worksheets' objective boxes, 77 topics) + CED codes collapsed
  (`data/learning-objectives.json` ← `apstat_*_framework.md`, 177 LOs). Desk 🎯 "This Week" app (`?ced=0` student view).
- **School bulletin is its own desktop app** (`f2756b07`): 📌 icon + window + soon-count badge; the strip over the Do Now card
  is gone; `data/bulletin.json` workflow unchanged. Never re-add a strip.
- **SAT Day Wed 10-21 = Period E break day** (`917e306c`): first time BOTH sections carry breaks → `generateSchedule` now
  merges two single-break runs so neither break moves the other section (B byte-identical; E absorbs via one extra
  catch-up pair). Both schedule JSONs + m2b golden regenerated; E due dates from 10-21 shift one meeting.
- Repaired the 3 stale Teacher Tools launcher tests; GitNexus reindexed (stale again after the last pushes).
- Earlier in the window (09-09 → 09-21, from memory, not in this file before): rubric slim course-wide (`7b8324b7`), lesson
  zero lag = class day + 13 (`dueLagDays`), local answer re-hydration (`1c3e867a`), APStat Park three doors, weekly
  auto-published misconception DOK sheet (Fri 21:00 / Sat 07:00 on Athena, `e5a54c21`), DOK student sheets print the E
  criteria (`8382daec`), B Work Days, Schoology best-wins, exit-ticket bonus, E pairing v2, Schoology sync LIVE (B 16:30 /
  E 16:45 on Athena), Desk school bulletin (`9a475e1c`).

### Gotchas learned this session
- The scheduled Schoology sync runs the WORKING TREE at 16:30/16:45 — a fix committed in the evening ships the next day.
- `generateSchedule`'s reference-run trick assumed only ONE section has breaks; the two-run merge is the general rule now.
- Codex leaves CRLF in files it touches (`expected.json` this time); `core.autocrlf=input` normalizes at commit.
- GitNexus does not index the Desk's inline functions — impact on `_zeroWarnings` etc. must be done by grep.
- The Desk auto-arranges desktop icons by usage (`bumpUsage`); markup positions are only the initial layout.

---

## ⏭ SESSION 40 (2026-09-04, Athena laptop) — superseded by s41 above. DOK LADDERS COMPLETE — 66/66.

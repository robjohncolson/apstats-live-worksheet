...
// The "✨ AI re-check" button (manual trigger). Inserted right after the
// worksheet's unique "Check Answers" button.
export const RECHECK_BUTTON =
  '<button class="btn-ai-recheck" onclick="aiGradeWorksheet({manual:true})" ' +
  'title="Re-grade with AI — accepts answers that mean the same thing, and re-checks your written answers (only ever raises your grade)">' +
  '&#10024; AI re-check</button>';
...
export const INJECTED_JS = String.raw`        // ==================== AI WORKSHEET GRADING (semantic blanks + folded FRQ) ====================
        ...
        (function () {
            ...
            async function _aiGradeFrqs(manual) {
                if (typeof gradeReflection !== 'function') return;   // some worksheets grade FRQs inline
            ...
            function _aiSetBusy(busy) {
                try {
                    ...
                    if (!btn) return;
                    btn.disabled = busy;
                    btn.innerHTML = busy ? '✨ grading…' : '✨ AI re-check';
                } catch (_) {}
            }
            ...
            window.aiGradeWorksheet = async function (opts) {
                ...
                try {
                    ...
                    try { await _aiGradeFrqs(manual); } catch (_) {}
                    window._aiLastGradedHash = hash;
                    if (manual) _aiToast('AI check complete.');
                } finally {
...
export function wireHtml(raw) {
  ...
  // 1) Button — insert right after the unique "Check Answers" button.

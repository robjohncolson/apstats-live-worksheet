...
export const INJECTED_JS = String.raw`        // ==================== AI WORKSHEET GRADING (semantic blanks + folded FRQ) ====================
        ...
        (function () {
            ...
            async function _aiGradeFrqs(manual) {
                if (typeof gradeReflection !== 'function') return 0;   // some worksheets grade FRQs inline
            ...
            function _aiUpdateScoreDisplay() {
                try {
                    ...
                    var s = _aiBlankStats();
                    if (s.filled > 0) {
                        var pct = Math.round((s.correct / s.filled) * 100);
                        el.textContent = 'Score: ' + s.correct + '/' + s.filled + ' (' + pct + '%)' +

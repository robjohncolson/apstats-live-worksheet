...
export const BLOCK_VERSION = 'FRQ_GRADE_TICKETS_C_V1';
...
export const INJECTED_JS = String.raw`        // ==================== AI WORKSHEET GRADING (semantic blanks + folded FRQ) ====================
        // ${SENTINEL} — AI_WORKSHEET_GRADING_BUILD.md — ${BLOCK_VERSION}.
        ...
        (function () {
            ...
            function _aiIsAuthoritative() {
                return _aiFrqConfig.mode === 'authoritative' && _aiFrqConfig.authoritative === true;
            }
                ...
                try {
                    if (typeof gradingState !== 'undefined' && gradingState) {
                        ...
                        gradingState.set(taId, {
                            ...
                            appealCount: previous.appealCount || 0,
            ...
            function _aiLastSeenGradedKey(itemId) {
                ...
                return who ? 'apstats_frq_last_seen_graded_' + who + '_' + itemId : null;
            }
            ...
            async function _aiRenderFrqStatus(itemId, item) {
                try {
                    ...
                    var hasStoredGrade = status === 'graded' || status === 'appeal-queued' || status === 'appeal-grading';
                    if (hasStoredGrade) {
                        ...
                        if (status === 'graded') {
                            ...
                            if (_aiFrqAnnouncedGrade[itemId] === announcementKey) return;
                            if (away) {
            ...
            async function _aiSubmitAuthoritativeAppeal(questionId) {
                ...
                try {
                    ...
                    _aiFrqStates[itemId] = { status: 'appeal-queued' };
                    await _aiRenderFrqStatus(itemId, _aiFrqStates[itemId]);
            ...
            try {
                _aiAddOwnedListener(window, 'storage', function (e) {
                    if (e && e.key === 'apstats_roster.v1') _aiSetOwnedTimeout(_aiOnLoad, 600);
                });
            } catch (_) {}

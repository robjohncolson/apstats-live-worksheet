...
for (const period of ['B', 'E']) {
  ...
  const got = placed[period].filter(item => item.kind !== 'work' && item.kind !== 'break');
...
const calendar = {
  ...
  breakDays: Object.fromEntries(['B', 'E'].map(period => [period,
    placed[period].filter(item => item.kind === 'break').map(item => item.date)])),

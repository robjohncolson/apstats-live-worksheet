// curriculum-charts.js — lifted from curriculum_render/js/charthelper.js + charts.js
// Provides window.getChartHtml and window.renderChartNow for study_guide_diagnostic.html.
// Do NOT modify function bodies — keep diffs against the source minimal.
// Depends on: chart.min.js, chartjs-plugin-datalabels.min.js (loaded before this file).

const TANGO_CHART_COLORS = {
    lightPalette: [
        '#A8330A', '#2F5A5E', '#25663F', '#8A6D12', '#B03A2E',
        '#4D4A78', '#7A3B69', '#315B86', '#6B4B2A', '#465B2B'
    ],
    nightPalette: [
        '#FF8A50', '#AECACD', '#91D3A7', '#E6C96A', '#F2A09A',
        '#C9B6E4', '#8FB9E3', '#D6A2C5', '#B8C97D', '#D3D1C2'
    ],
    text: '#161616',
    nightText: '#F3F1E8',
    grid: '#6E6C62',
...
function generateChartColors(count) {
    ...
    const colors = isDark ? TANGO_CHART_COLORS.nightPalette : TANGO_CHART_COLORS.lightPalette;
    const result = [];
...
function getTextColor() {
    ...
    return color || (isDarkMode() ? TANGO_CHART_COLORS.nightText : TANGO_CHART_COLORS.text);
}
...
function getGridColor() {
    ...
    const color = getCSSVariable(varName);
    return color || (isDarkMode() ? TANGO_CHART_COLORS.nightGrid : TANGO_CHART_COLORS.grid);
}
...
function getScatterPointColor() {
    ...
    const color = getCSSVariable(varName);
    return color || (isDarkMode() ? TANGO_CHART_COLORS.nightAccent : TANGO_CHART_COLORS.accent);
}
...
function getChartHtml(chartData, canvasId) {
    ...
    return `
        <div class="chart-container" ${containerStyle}>
            ...
            ${config.description ? `<div style="font-size: 0.9em; color: var(--sg-text-dim, ${TANGO_CHART_COLORS.muted}); text-align: center; margin-bottom: 10px; font-style: italic;">${config.description}</div>` : ''}
    ...
    const config = chartData.chartConfig || {};
        ...
        if (chartData.chartType === 'bar' || chartData.chartType === 'histogram') {
            ...
            const datasets = chartData.series.map((series, index) => {
                const color = colorPalette[index % colorPalette.length] || getScatterPointColor();
                return {
                    label: series.name,
                    data: series.values,
            ...
            if (isPolarArea) {
                ...
                datasetConfig.borderColor = isDarkMode() ? TANGO_CHART_COLORS.nightPaper : TANGO_CHART_COLORS.paper;
            ...
            if (config.referenceLineAtZero) {
                ...
                datasets.push({
                    ...
                    borderColor: TANGO_CHART_COLORS.bad,
                    borderDash: [6, 4],
                    borderWidth: 2,
                    fill: false,
                    pointRadius: 0,
            ...
            const datasets = dfList.map((df, idx) => {
                ...
                const color = colorPalette[idx % colorPalette.length] || getScatterPointColor();
                return {
                    label: labels[idx],
                    type: 'line',
                    data: pts,
...
// Also expose directly on window so attachMedia in study_guide_diagnostic.html
// can call window.getChartHtml / window.renderChartNow with a simple guard.
window.getChartHtml = getChartHtml;
window.renderChartNow = renderChartNow;

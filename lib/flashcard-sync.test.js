...
describe('FlashcardSync — pull-time 409 reconciliation', () => {
  it('re-GETs, merges and PUTs once more when another device wrote between our GET and our PUT', async () => {
    ...
    const storage = memoryStorage({ ['apstats_srs_log_' + EMAIL]: JSON.stringify([entry({ roundId: 'a-1', ts: 1_700_000_000_100 })]) });
    ...
    const localKeys = JSON.parse(storage.getItem('apstats_srs_log_' + EMAIL)).map(FlashcardSync.entryKey).sort();

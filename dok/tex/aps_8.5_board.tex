...
\begin{description}[leftmargin=1.15in, labelwidth=1in, itemsep=2pt, topsep=2pt]
  \item[\dokbadge{1}~(a)] For each design, state the number of random samples and the categorical variable or variables recorded.
  \item[\dokbadge{2}~(b)] Calculate the expected counts and $df$, then verify random sampling, 10 percent, and expected counts for both designs.
  \item[\dokbadge{3}~(c)] Adjudicate the claims. For each design, name the procedure and state $H_0$ and $H_a$ in context. Use the sampling structure, diagnose the rejected table-only rule, and identify the numerical pieces that remain identical.
\end{description}

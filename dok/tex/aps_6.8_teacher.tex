...
\daybanner{AP Statistics \textperiodcentered\ Unit 3 \textperiodcentered\ Topic 3.10 \textperiodcentered\ TEACHER KEY}{Two Ways to Build an Interval}
...
\begin{itemize}[leftmargin=1.5em,itemsep=2pt,topsep=2pt]
  \item \textbf{judgment-1} (required) --- Chooses Elena using independent samples and adding variance contributions under one square root.
  \item \textbf{judgment-2} (required) --- Explains that separate intervals are not the requested 95 percent procedure for the difference.
  \item \textbf{judgment-3} (required) --- Uses Elena's positive interval versus Devon's interval containing zero to explain the changed inference.
\end{itemize}
...
The two reported ranges disagree about whether the towns could be equal. Which range would you trust, and why?\par
...
For two independent random samples, use a two-sample $z$-interval for $p_1-p_2$:
$(\hat p_1-\hat p_2)\pm z^*\sqrt{\hat p_1(1-\hat p_1)/n_1+\hat p_2(1-\hat p_2)/n_2}$.
For 95\% confidence, $z^*=1.96$. Check independent random samples or random assignment,
each $n_i\leq0.10N_i$ when sampling without replacement, and at least 10 observed successes and failures in each group.
Independent estimated variances add under one square root. Confidence belongs to the procedure used to construct the interval.
An interval for a difference that includes 0 leaves equal proportions plausible; one entirely above 0 supports a positive difference.
\end{callout}
...
\dokbadge{3}~\textbf{(c)} In 3--4 sentences, choose a method using the independent samples and the variance formula. Explain why subtracting separate 95\% intervals does not establish a 95\% interval for the difference. Use zero's location to explain how the method changes the conclusion.\par

...
**Status:** DRAFT for review (2026-06-22, rev 2). No code yet.
...
- **Acquisition pipeline** — `scripts/fetch-offline-videos.mjs` (teacher-run, online, one-time): parse the ~144 Drive `altUrl`s out of the Desk's video data, download each (the teacher's own files; `gdown`/yt-dlp under the teacher's session), the 1 YouTube link via yt-dlp, and any other source. Output a `media/` tree + `media-manifest.json` mapping `lesson → local relative path`. Idempotent/resumable (skip already-downloaded; a multi-GB pull will be interrupted).

...
export function classifyUngradedFrqRows(snapshot, manifest, options = {}) {
  ...
  for (const student of snapshot.students || []) {
    ...
    for (const record of student.bundle?.records || []) {
      ...
      } else if (options.regradeLow) {
        // --regrade-low: re-send already-graded P (0.5) and I (0) rows through the
        // grader under the CURRENT rubric bundle. The server's FRQ floor keeps a
        // lower verdict from ever replacing a higher one, so this can only raise.
        if (!hasScore || Number(record.score) >= 1) continue;
...
export function parseArgs(argv) {
  ...
  if (options.backfillFeedback && options.regradeLow) {
    throw new Error('--backfill-feedback and --regrade-low are mutually exclusive');
  }

...
(function () {
  ...
  const ROSTER_KEY = 'apstats_roster.v1';

  // Per-student state (TI84_TRAINER_STUDENT_STATE_SPEC.md): trainer progress
  // is keyed by roster studentId, with a signed-out 'anon' fallback, so shared
  // classroom devices never blend one student's SRS/mastery/seeds into
  // another's. The bare legacy keys are frozen and copied to anon once.
  function resolveStudentId() {
    try {
      const id = window.rosterClient?.current?.()?.studentId;
  ...
  let activeKeyId = resolveStudentId();
  const stateKey = () => `${STORAGE_KEY}.${activeKeyId}`;
  const memKey = () => `${MEMORY_KEY}.${activeKeyId}`;

  // One-time: seed a fresh anon slot from the pre-namespacing device blob so
  // the device's history survives for signed-out use. No student inherits it
  // implicitly (spec §2.1) — only anon.
  function migrateLegacyToAnon() {
    if (activeKeyId !== 'anon') {
      return;
    }
    try {
      const legacyState = window.localStorage.getItem(STORAGE_KEY);
      if (legacyState !== null && window.localStorage.getItem(stateKey()) === null) {
        window.localStorage.setItem(stateKey(), legacyState);
      }
      const legacyMem = window.localStorage.getItem(MEMORY_KEY);
      if (legacyMem !== null && window.localStorage.getItem(memKey()) === null) {
        window.localStorage.setItem(memKey(), legacyMem);
      }
    } catch (error) {
      console.warn('Legacy trainer state migration skipped.', error);
    }
  }
  migrateLegacyToAnon();
  ...
  function checkPropertyField(rule, rawInput, values, parsedByKey) {
    // Accept the calculator's EXACT display format — students copy the
    // screen, so "{10 29 14 22 26}" must parse the same as "10, 29, 14...".
    // Braces and a trailing ellipsis (long lists scroll) are stripped.
  ...
  function savePersisted() {
    // Pre-save recheck (spec §1): if the signed-in student changed since the
    // last resolve, hand off cleanly rather than writing this session under
    // the wrong key. The switch persists the OLD state itself, so return.
    if (maybeSwitchIdentity()) {
      return;
    }
    app.persisted.filterUnit = app.filterUnit;
    try {
      window.localStorage.setItem(stateKey(), JSON.stringify(app.persisted));
    } catch (error) {
  ...
  function saveListMemory() {
    try {
      window.localStorage.setItem(memKey(), JSON.stringify(listMemory));
    } catch (error) {
      if (!storageWarningShown) {
        storageWarningShown = true;
        app.banner = "Progress can't be saved in this browser.";
  ...
  // Hand off to a new signed-in identity WITHOUT merging (spec §1/§2): flush
  // the current student's state to their own key, reset the live session, then
  // reboot the persisted layers from the new key. Returns true if it switched.
  function maybeSwitchIdentity() {
    const nextId = resolveStudentId();
    if (nextId === activeKeyId) {
      return false;
    }

    // Flush the OUTGOING student's state under the OLD key directly — never
    // via savePersisted (which would re-resolve and write under the new id).
    try {
      app.persisted.filterUnit = app.filterUnit;
      window.localStorage.setItem(stateKey(), JSON.stringify(app.persisted));
      window.localStorage.setItem(memKey(), JSON.stringify(listMemory));
    } catch (error) {
      console.warn('Could not flush outgoing trainer state.', error);
    }

    activeKeyId = nextId;
    migrateLegacyToAnon();

    // Fresh session for the incoming student — no walkthrough/handheld/practice
    // state carries across an identity boundary.
    app.persisted = loadPersisted();
    app.filterUnit = app.persisted.filterUnit ?? 'all';
  ...
  // shared roster key; window focus catches a switch that happened while the
  // trainer was hidden. Either way, hand off without merging and re-render.
  window.addEventListener('storage', (event) => {
    if (event.key === ROSTER_KEY && maybeSwitchIdentity()) {
      render();
    }
  });

  window.addEventListener('visibilitychange', () => {
    if (!document.hidden && maybeSwitchIdentity()) {
      render();
    }
  });

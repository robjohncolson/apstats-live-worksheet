...
(function () {
  ...
  function confirmDataSetup() {
    ...
    // reads a wrong result. The ROM can only be loaded via keystrokes, so route
    // "I'm done" through the auto-fill that actually types the data in. (Mock
    // mode reads from the native sim, so the plain sync is correct there.)
  ...
  function logVerifyDiagnostic(label, procedureId, problem, results) {
    try {
      const summary = Object.entries(results)
        ...
        .join(' | ');
      // For data procedures, dump the lists so we can tell a wrong calculator
      // result apart from a wrong read: native L1 is what the trainer THINKS is
      // loaded; compare against the problem's data.
      let lists = '';
      const v = problem?.values ?? {};
      const expectedData = v.data ?? v.L1 ?? v.observed ?? null;
      if (Array.isArray(expectedData) && app.bridge?.getList) {
        lists = ` | L1(native)=[${(app.bridge.getList('L1') ?? []).join(',')}]`
          + ` | expectedData=[${expectedData.join(',')}]`;
      }
      console.warn(`[TI84-VERIFY] ${label} ${procedureId} — ${summary}${lists}`);
    } catch (_) {

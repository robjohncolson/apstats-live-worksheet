...
The local agent remains disabled pending the teacher's funding-account choice.
Read-only preflight has verified the existing `allowance` account can cover the
rejected plan and configured fee buffer.
...
Code is kept local under the session's no-push instruction. No database migration
or server deployment is needed for the sender fix or completed batch recovery.
